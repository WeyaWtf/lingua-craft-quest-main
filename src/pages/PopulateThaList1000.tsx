...
export default function PopulateThaList1000() {
  ...
  const populatePath = async () => {
    ...
    try {
      ...
      const { data: pathData, error: pathError } = await supabase
        .from('learning_paths')
        .select('*')
        .eq('id', 'path-thai-list-1000-words')
        .single();

      if (pathError || !pathData) {
        toast.error('Parcours THA LIST 1000 introuvable');
        setLoading(false);
        return;
      }

      // Récupérer tous les exercices THA
      const { data: exercises, error: exError } = await supabase
        .from('exercises')
        .select('id, title')
        .ilike('title', 'THA LIST 1000 - Bundle%')
        .order('title');
      ...
      console.log(`✅ Found ${exercises.length} THA exercises`);
      ...
      toast.success('Parcours THA LIST 1000 peuplé avec succès !');
    } catch (err) {
  ...
  return (
    <div className="container mx-auto p-8 max-w-4xl">
      <h1 className="text-3xl font-bold mb-6">🇹🇭 Populate THA LIST 1000 WORDS</h1>

      <Card className="p-6 mb-6">
        <h2 className="text-xl font-semibold mb-4">Ce script va :</h2>
        <ul className="space-y-2 text-sm list-disc list-inside">
          <li>Récupérer tous les exercices "THA LIST 1000 - Bundle X"</li>
      ...
      {!loading && results.length === 0 && (
        <Card className="p-6">
          ...
          <p className="text-gray-700 mb-6">
            Assurez-vous que tous les exercices THA Bundle (1-40) ont été créés avant de lancer cette opération.
          </p>
          <Button
            ...
            className="w-full"
          >
            🚀 Peupler le parcours THA LIST 1000
          </Button>
        </Card>
      )}
      ...
      {!loading && results.length > 0 && (
        <>
          ...
          <Card className="p-6 bg-green-50 border-green-200">
            ...
            <p className="text-gray-700 mb-6">
              Le parcours "THA LIST 1000 WORDS" a été mis à jour avec tous les exercices disponibles.
            </p>
